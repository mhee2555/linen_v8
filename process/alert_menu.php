...
if (!empty($_POST['FUNC_NAME'])) {
  ...
  } else if ($_POST['FUNC_NAME'] == 'alert_Sig') {
    alert_Sig($conn);
  } else if ($_POST['FUNC_NAME'] == 'showDetailSig') {
    showDetailSig($conn);
  }
...
function alert_Sig($conn)
{
  ...
  $wheresite = "";
  // if ($PmID == 8) {
    $wheresite = "AND shelfcount.SiteCode = '$hotpCode' ";
  // }

  $wheredep = "";
  if ($PmID == 8) {
    $wheredep = "AND shelfcount.DepCode = '$DepCode' ";
  }
  ...
  $Sql = "SELECT
          ...
          shelfcount
          INNER JOIN department ON shelfcount.DepCode = department.DepCode
          WHERE shelfcount.IsStatus = 3  $wheresite  $wheredep AND  shelfcount.signStart != 'null' AND shelfcount.signature  is NULL ";
...
function showDetailSig($conn)
{
  ...
  $Sql = "SELECT
        item.ItemName   ,
        shelfcount_detail.TotalQty AS pay
        FROM
        shelfcount
        INNER JOIN department ON shelfcount.DepCode = department.DepCode
        ...
        WHERE shelfcount.DocNo = '$DocNo' AND shelfcount.IsStatus = 3 AND  shelfcount.signStart != 'null' AND shelfcount.signature  is NULL AND shelfcount_detail.TotalQty > 0 ";
