...
function ShowItem($conn, $DATA)
{
  $HptCode = $DATA['HptCode'];
  $count = 0;
  $Select = "SELECT te.ID, te.HptCode, sc_time_2.TimeName , site.HptName
    FROM sc_express te
...
function EditItem($conn, $DATA)
{
  ...
  $HptCode = $DATA['HptCode'];
  $Time = $DATA['Time'];
  $Sql = "UPDATE sc_express SET time_value = '$Time' , Modify_Date = NOW() , Modify_Code =  $Userid    WHERE ID = $ID";
  mysqli_query($conn, $Sql);
  ShowItem($conn, $DATA);
}
