...
for ($r = 0; $r < $count; $r++) {
    ...
    $query = "SELECT
  coalesce(kpi_dirty1_detail.IsCheck,'-') AS  IsCheck
  FROM
  kpi_dirty1_detail
...
$objPHPExcel->getActiveSheet()->getStyle("A6:" . $date_cell[$lastdate] . "24")->applyFromArray($styleArray);
...
$objPHPExcel->getActiveSheet()->getStyle("E27")->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);


$writer = new Xlsx($objPHPExcel);

// ชื่อไฟล์
$objPHPExcel->getActiveSheet()->setTitle('Report_QC_Process_dirty');
...
$file_export = "Report_QC_Process_checklist_dirty" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
