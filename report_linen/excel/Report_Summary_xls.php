...
if ($chk == 'one') {
  if ($format == 1) {
    ...
    $where2 =   "WHERE DATE (sc.Docdate) = DATE('$date1')";
    ...
    $where2 =   "WHERE year (sc.Docdate) LIKE '%$date1%'";
  ...
  $where2 =   "WHERE sc.Docdate BETWEEN '$date1' AND '$date2'";
  ...
  $where2 =   "WHERE month (sc.Docdate) = " . $date1;
  ...
  $where2 =   "WHERE DATE(sc.DocDate) BETWEEN '$betweendate1' AND '$betweendate2'";
...
if ($itemfromweb == '0') {
  ...
  foreach ($DepCode as $keyDep => $DepCodeloop) {
    ...
    $item = "SELECT
                      ...
                      $where2
                      AND item.ItemCode IN (  ";
    foreach ($itemCode as $keyitemcode => $itemcodeloop) {
      $item .= "  '$itemcodeloop' ,";
    }
    $item = rtrim($item, ' ,');
    $item .= " )  AND sc.DepCode = '$DepCodeloop'
