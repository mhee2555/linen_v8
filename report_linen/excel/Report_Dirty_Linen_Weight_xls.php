...
if ($chk == 'one') {
    ...
    $where =   "WHERE YEAR(dirty.DocDate) = '$year1' AND month (dirty.Docdate) = '$date1'";
    $datetime = new DatetimeTH();
    if ($language == 'th') {
...
for ($sheet = 0; $sheet < $count_sheet; $sheet++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A8', $array2['factory'][$language]);
    $objPHPExcel->getActiveSheet()->setCellValue('B8', $array2['itemname'][$language]);
    ...
    foreach ($DepCode as $keyDep => $DepCodeloop) {
        $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$r] . $start_row, $DepName[$keyDep]);
        $r++;
        ...
        if ($chk == 'between' || $chk == 'month') {
            $data = "SELECT   COALESCE(SUM(dirty_detail.Qty),'0') AS Totalqty,
                                ...
                                dirty.Time_ID AS Time_ID_chk
                       FROM         ( SELECT DocNo, DepCode, Qty,ItemCode , Weight FROM dirty_detail ) AS dirty_detail
                      INNER JOIN ( SELECT DocNo, IsStatus, FacCode, Time_ID, DocDate FROM dirty ) AS dirty ON dirty.DocNo = dirty_detail.DocNo
                      INNER JOIN ( SELECT FacCode FROM factory ) AS factory ON factory.FacCode = dirty.FacCode
                      INNER JOIN ( SELECT DepCode, HptCode FROM department ) AS department ON department.DepCode = dirty_detail.DepCode
                      INNER JOIN ( SELECT HptCode FROM site ) AS site ON site.HptCode = department.HptCode WHERE DATE( dirty.DocDate ) IN (";
            foreach ($date as $key => $dateval) {

                $data .= " '$dateval' ,";
            }
            $data = rtrim($data, ' ,');
            $data .= " ) 	AND dirty.isStatus <> 9 
                                        AND dirty.isStatus <> 0 
                                        AND dirty.Time_ID IN ( ";
            foreach ($Time_ID as $keyTime_ID => $Time_IDval) {
            ...
            $data .= " )  AND dirty.Faccode = '$FacCode'
                                        AND site.HptCode = '$HptCode'
                                        AND dirty_detail.DepCode = '$DepCodeloop'
                                         $dirty_time  ";
            if ($sheet <> 0) {
                $data .= " AND dirty_detail.ItemCode = '$sheet_item[$sheet]'";
            }

            $data .= "GROUP BY  DATE( dirty.DocDate ) , dirty.Time_ID 
                      ORDER BY dirty.DocDate ASC , dirty.Time_ID ASC";
            ...
            while ($Result = mysqli_fetch_assoc($meQuery)) {
                ...
                $Time_ID_chk[$cnt] =  $Result["Time_ID_chk"];
                $cnt++;
            }
            $sumdayweight = 0;
            $sumdaytotal = 0;
            ...
            foreach ($date as $key => $val) {
                ...
                $TotaldayTotalqty += $sumdaytotal;
                $TotaldayWeight += $sumdayweight;
            }

            $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$r] . $start_row, $sumdaytotal);
            $r++;
            ...
            for ($code = 0; $code < $count; $code++) {
                $data = "SELECT   COALESCE(SUM(dirty_detail.Qty),'0') AS Totalqty,
                      ...
                      INNER JOIN ( SELECT FacCode FROM factory ) AS factory ON factory.FacCode = dirty.FacCode
                      INNER JOIN ( SELECT DepCode, HptCode FROM department ) AS department ON department.DepCode = dirty_detail.DepCode
                      INNER JOIN ( SELECT HptCode FROM site ) AS site ON site.HptCode = department.HptCode ";
                if ($chk == 'one') {
                    if ($format == 1) {
    ...
    FROM
    ( SELECT DocNo, DepCode, Qty, ItemCode ,  Weight FROM dirty_detail ) AS dirty_detail
    INNER JOIN ( SELECT DocNo, IsStatus, FacCode, Time_ID, DocDate FROM dirty ) AS dirty ON dirty.DocNo = dirty_detail.DocNo
    INNER JOIN ( SELECT FacCode FROM factory ) AS factory ON factory.FacCode = dirty.FacCode
    INNER JOIN ( SELECT DepCode, HptCode FROM department ) AS department ON department.DepCode = dirty_detail.DepCode
    INNER JOIN ( SELECT HptCode FROM site ) AS site ON site.HptCode = department.HptCode
...
foreach ($DepCode_show as $keyDep => $DepCodeloop) {
    $query = " SELECT
                ...
                AND dirty.FacCode = '$FacCode' 
                AND dirty_detail.RequestName <> ''
                AND department.DepCode = '$DepCodeloop'
    ...
    while ($Result = mysqli_fetch_assoc($meQuery)) {
        ...
        $r++;
        $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$r] . $start_row, $DepName_show[$keyDep]);
        $r++;
        ...
        if ($chk == 'between' || $chk == 'month') {
            $data = "SELECT   COALESCE(SUM(dirty_detail.Qty),'0') AS Totalqty,
                          ...
                          DATE( dirty.DocDate ) AS Date_chk,
                          dirty.Time_ID AS Time_ID_chk
                 FROM         ( SELECT DocNo, DepCode, Qty,ItemCode , RequestName , Weight FROM dirty_detail ) AS dirty_detail
                INNER JOIN ( SELECT DocNo, IsStatus, FacCode, Time_ID, DocDate FROM dirty ) AS dirty ON dirty.DocNo = dirty_detail.DocNo
                INNER JOIN ( SELECT FacCode FROM factory ) AS factory ON factory.FacCode = dirty.FacCode
                INNER JOIN ( SELECT DepCode, HptCode FROM department ) AS department ON department.DepCode = dirty_detail.DepCode
                INNER JOIN ( SELECT HptCode FROM site ) AS site ON site.HptCode = department.HptCode WHERE DATE( dirty.DocDate ) IN (";
            ...
            // echo $data;
            // echo "</pre>";
            $meQuery1 = mysqli_query($conn, $data);
            while ($Result1 = mysqli_fetch_assoc($meQuery1)) {
                $aTotalqty[$cnt] =  $Result1["Totalqty"];
                $aWeight[$cnt] =  $Result1["Weight"];
                $Date_chk[$cnt] =  $Result1["Date_chk"];
                $Time_ID_chk[$cnt] =  $Result1["Time_ID_chk"];
                $cnt++;
            }
            $sumdayweight = 0;
            $sumdaytotal = 0;
            ...
            for ($code = 0; $code < $count; $code++) {
                ...
                while ($Result1 = mysqli_fetch_assoc($meQueryx)) {
                    $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$r] . $start_row, $Result1["Totalqty"]);
                    $r++;
                    $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$r] . $start_row, $Result1["Weight"]);
                    $r++;
                    $sumdayTotalqty += $Result1["Totalqty"];
                    $sumdayWeight += $Result1["Weight"];
                }
                $TotaldayTotalqty += $sumdayTotalqty;
                $TotaldayWeight += $sumdayWeight;
            }
...
$file_export = $array2['r1']['en'] . "_" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
