...
if ($chk == 'one') {
    ...
    $where =   "WHERE month (damagenh.Docdate) = " . $date1 . " AND YEAR(damagenh.Docdate) =" . $year1 ; 
    $datetime = new DatetimeTH();
    if ($language == 'th') {
...
for ($day = 0; $day < $count; $day++) {
    $data =       "SELECT
              COALESCE(SUM(damagenh_detail.Qty),'0') AS Totalqty,
                      COALESCE(SUM(damagenh_detail.Weight),'0') AS Weight
              FROM
              damagenh_detail
              INNER JOIN damagenh ON damagenh.DocNo = damagenh_detail.DocNo
...
$file_export = $array2['DM']['en'] . "_" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
//
